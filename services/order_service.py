...
class OrderService(BaseServiceImpl):
    ...
    def save(self, schema: OrderSchema) -> OrderSchema:
        ...
        if not self._client_repository.exists(schema.client_id):
        ...
        if not self._bill_repository.exists(schema.bill_id):
            logger.error(f"Bill with id {schema.bill_id} not found")
            raise InstanceNotFoundError(f"Bill with id {schema.bill_id} not found")

        try:
            delivery_method = DeliveryMethod(schema.delivery_method).value
            status = Status(schema.status).value
        except ValueError:
            raise ValueError("Delivery method o status inválido")
        ...
        schema = schema.model_copy(update=update_data)
